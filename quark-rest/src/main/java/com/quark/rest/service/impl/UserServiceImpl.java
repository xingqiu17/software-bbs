...
//        loginId.remove(user.getId());//维护一个登录用户的set
    ...
    @Override
    public void updateUserSchool(String token, String school) {
        User cacheUser = redisService.getString(REDIS_USER_KEY + token);
        if (cacheUser == null) {
            throw new ServiceProcessException("用户Session过期，请重新登录");
        }
        ...
        User user = repository.findOne(cacheUser.getId());
        user.setSchool(school); // 假设字段名为 school
        repository.save(user);

        // 更新缓存
        cacheUser.setSchool(school);
        redisService.cacheString(REDIS_USER_KEY + token, cacheUser, REDIS_USER_TIME);
    }
// ----------------------- new_end ---------------------------------------
