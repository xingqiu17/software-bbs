...
public class UserController extends BaseController {
    ...
    public QuarkResult Login(String email,String password) {
        ...
        QuarkResult result = restProcessor(() -> {
            ...
            if (loginUser == null)
                return QuarkResult.warn("用户不存在，请重新输入");
    ...
    @ApiOperation("根据Token修改用户的学校")
    @ApiImplicitParams({
        @ApiImplicitParam(name = "token", value = "发送给用户的唯一令牌", dataType = "String"),
        @ApiImplicitParam(name = "school", value = "学校名称", dataType = "String")
    })
    @PutMapping("/school/{token}")
    public QuarkResult updateUserSchool(@PathVariable("token") String token, String school) {
        QuarkResult result = restProcessor(() -> {
            if (school == null || school.trim().isEmpty()) {
                return QuarkResult.warn("学校名称不能为空");
            }

            // 可以在这里校验学校名称是否合法
            // 比如: 校验是不是在已知学校列表里
            // if (!schoolService.isValidSchool(school)) {
            //     return QuarkResult.warn("无效的学校名称");
            // }

            userService.updateUserSchool(token, school);
            return QuarkResult.ok();
        });
    ...
    public QuarkResult getUserById(@PathVariable("userid") Integer userid,@RequestParam("token") String token){
        QuarkResult result = restProcessor(() -> {
